...
add_action( 'add_meta_boxes', 'pk_reservation_add_custom_boxes' ); // quand wordpress charge les meta boxes, il viendra charger notre custom box
    ...
    // Vérifie si la date de début de réservation a été changée
    if ( isset( $_POST['pk_reservation_begin_field'] ) ) {
        // Sanitization de la valeur de date de début de réservation
        $new_begin_date = sanitize_text_field( $_POST['pk_reservation_begin_field'] );
    }
    
    // Vérifie si la date de fin de réservation a été changée
    if ( isset( $_POST['pk_reservation_end_field'] ) ) {
        // Sanitization de la valeur de date de fin de réservation
        $new_end_date = sanitize_text_field( $_POST['pk_reservation_end_field'] );
    }
    ...
    if ( $new_begin_date && $new_end_date ) { // il faut que les deux existent
        // conversion au format timestamp
    ...
    if ( $end_timestamp < $begin_timestamp ) // si le timestamp (la date) de fin est inférieur à celui du début
    {
        ...
        //Génère une erreur : la date de fin arrive avant la date de début
        $new_begin_date = null;
        $new_end_date = null;
    }

    // Enregistre les dates de réservation si elles sont renseignées (!= null)
    if ( ! is_null( $new_begin_date ) ) {
        update_post_meta( $post_id, '_pk_reservation_begin_date', $new_begin_date );
    }
    if ( ! is_null( $new_end_date ) ) {
